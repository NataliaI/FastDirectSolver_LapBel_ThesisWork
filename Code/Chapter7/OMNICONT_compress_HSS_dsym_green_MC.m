function [Atilde] = OMNICONT_compress_HSS_dsym_green_MC(C,nbox_max,acc,dist_rel, nproxy,N,M, centers)
...
for ibox = nboxes:(-1):2
    ...
    % Determine a circle that circumscribes Gamma_tau (indskel):
    ...
    if dist<2  % obtaining indices of points in the near and far field
        ...
        % obtaining coordinates of far-field : (may cause the algorithm to slow
        %                                       down a bit)
    ...
    if isempty(LIST_NEI{ibox})|| isempty(indskel_inside)  % if ibox has no neighbours listed/ no near field
        A21proxy = [OMNICONT_construct_A_sing_vert(Cproxy, 1:nproxy, C, indskel, h, N, centers)]; % vertical block
        A12proxy = [OMNICONT_construct_A_sing_hor(C, indskel, Cproxy, 1:nproxy, h, N, centers, indfar)]; % horizontal block
    ...
    elseif dist >= 2 % if no proxy circle was created then entire section of 
        ...
        A21proxy = OMNICONT_construct_A_offd_MC(C, indskel_maybeinside, indskel, h, N, centers);
        A12proxy = OMNICONT_construct_A_offd_MC(C, indskel, indskel_maybeinside, h, N, centers);    
    else
        A21proxy = [OMNICONT_construct_A_offd_MC(C, indskel_inside, indskel, h, N, centers);...           % near field 
                    OMNICONT_construct_A_sing_vert(Cproxy, 1:nproxy, C, indskel, h, N, centers)];         % proxy points
        A12proxy = [OMNICONT_construct_A_offd_MC(C, indskel, indskel_inside, h, N, centers),...           % near field
                    OMNICONT_construct_A_sing_hor(C, indskel, Cproxy, 1:nproxy, h, N, centers, indfar)];  % proxy points
    end
...
Atilde = OMNICONT_construct_Atilde_MC(C, NODES, N, M, centers);
