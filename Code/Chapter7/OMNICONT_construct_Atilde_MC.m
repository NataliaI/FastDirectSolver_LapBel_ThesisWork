function Atilde = OMNICONT_construct_Atilde_MC(C,NODES,N,M, centers)
%--------------------------------------------------------------------------
...
for ibox = nboxes:(-1):2   % counting blocks from the bottom of the tree up
    if (NODES{5,ibox}==0)  % if block in tree (ibox) has no children (finest level)
        ...
        % building D:
        Dcell{1,dcount} = sparse(OMNICONT_construct_A_diag_MC(C,ind,h,N,centers));
...
for ibox = nboxes:(-1):1    % counting from the bottom of the tree up
    if (NODES{5,ibox}==2)   % if block in tree has 2 children
        ...
        sumkD = sumkD+sum;
        % building D^(l)
        Dcell{1,count} = sparse([zeros(k1) OMNICONT_construct_A_offd_MC(C,NODES{11,ison1},NODES{11,ison2},h,N,centers);...
                                 OMNICONT_construct_A_offd_MC(C,NODES{11,ison2},NODES{11,ison1},h,N, centers) zeros(k2)]);
...
Atilde(M*N+1:end,M*N+1:end) = Atilde(M*N+1:end,M*N+1:end)+Atilde_I;
