function [Atilde, NODES] = OMNICONT_compress_HSS_dsym_green_Sec3_6Cases(C,nbox_max,acc,dist_rel, nproxy,v)
%--------------------------------------------------------------------------
...
for ibox = nboxes:(-1):2
    ...
    if dist<2
        leg = legend([h_diag h_near h_far h_proxy], 'skeleton', '\Gamma_{near}', '\Gamma_{far}', '\Gamma_{proxy}');
    ...
    % *** for exporting:
    % export one example from each level:
    % addpath('export_fig-master');
    % if ibox == 8
    %     export_fig 'Case1ProxyEx1' -png -m4
    % elseif ibox == 4
    %     export_fig 'Case1ProxyEx2' -png -m4
    % elseif ibox == 2
    %     export_fig 'Case2ProxyEx' -png -m4
    % end
